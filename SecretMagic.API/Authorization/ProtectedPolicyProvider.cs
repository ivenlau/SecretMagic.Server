...
{
    public class ProtectedPolicyProvider : IAuthorizationPolicyProvider
    {
        const string POLICY_PREFIX = "ProtectedAPI";
        public DefaultAuthorizationPolicyProvider FallbackPolicyProvider { get; }

        public ProtectedPolicyProvider(IOptions<AuthorizationOptions> options)
        {
        ...
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            if (policyName.StartsWith(POLICY_PREFIX, StringComparison.OrdinalIgnoreCase))
            {
                ...
                policy.AddRequirements(new ProtectedRequirement(role));
